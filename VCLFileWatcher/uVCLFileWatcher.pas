...
type
  ...
  TVCLFileWatcher = class;
  TSFileExceptionEvent = procedure( Sender:TObject; excep:Exception;Error:Integer)of Object;
  ...
  public
    ...
    constructor Create(FParent:TVCLFileWatcher);
  ...
  TVCLFileWatcher = class (TComponent)
  private
    ...
    FOnException: TSFileExceptionEvent;
    ...
    procedure SetFileLastAcess(const Value: TSFileChangedEvent);
    procedure SetOnException(const Value: TSFileExceptionEvent);
  ...
  protected
    ...
    procedure doException(ex:Exception; Error:Integer);
  ...
  published
    ...
    property OnFileLastAccess: TSFileChangedEvent read FOnFileLastAccess write SetFileLastAcess;
    property OnException:TSFileExceptionEvent read FOnException write SetOnException;
...
constructor TSWatchDirThread.Create(FParent:TVCLFileWatcher);
begin
...
begin
  FHFile := CreateFile(pchar( FParent.FFileMonitoring ),
                      FILE_LIST_DIRECTORY OR GENERIC_READ,FILE_SHARE_READ OR FILE_SHARE_WRITE OR FILE_SHARE_DELETE,
                      NIL,OPEN_EXISTING,FILE_FLAG_BACKUP_SEMANTICS OR FILE_FLAG_OVERLAPPED,
                      0);
              ...
              if  ReadDirectoryChanges(FHFile,@ResBuffer,BufferLength,fParent.SubTree,FFilter,@ResBufferLength,@Overlapped,nil) then
                begin
                  ...
                  if ResWait = WAIT_FAILED then
                    begin
                      FParent.doException (nil,GetLastError);
                    end;
                    ...
                    begin
                      ...
                      repeat
                        ...
                        case ResStruct.Action of
                          ...
                          else FParent.doLastAcess(FileNameA+' vai saber ');
                        end;
...
constructor TVCLFileWatcher.Create(AOwner: TComponent);
begin
  inherited Create(AOwner);
  //FWatchThread := TSWatchDirThread.Create(Self);
end;

destructor TVCLFileWatcher.Destroy;
begin
...
procedure TVCLFileWatcher.doCriaded(sFileName: string);
begin
  if Assigned(FOnFileCriated)  then
    FOnFileCriated(Self,sFileName);
end;

procedure TVCLFileWatcher.doDeleted(sFileName: string);
begin
  if  Assigned(FOnFileDeleted)  then
    FOnFileDeleted(self,sFileName);
end;

procedure TVCLFileWatcher.doException(ex: Exception; Error: Integer);
begin
  if Assigned(FOnException) then
    FOnException(Self,ex,Error);
end;

procedure TVCLFileWatcher.doLastAcess(sFileName: String);
begin
...
procedure TVCLFileWatcher.doModified(sFileName: string);
begin
  if assigned(FOnFileModified) then
    FOnFileModified(Self,sFileName);
end;

procedure TVCLFileWatcher.doRenamed(sOldName, sNewName: string);
begin
...
procedure TVCLFileWatcher.ReLoadFileHandle(sFileName: string);
begin
  CloseHandle(FHFile);
...
procedure TVCLFileWatcher.SetAfterExecute(const Value: TNotifyEvent);
begin
  FAfterExecute := Value;
end;

procedure TVCLFileWatcher.SetFileLastAcess(const Value: TSFileChangedEvent);
begin
  FOnFileLastAccess := Value;
end;

procedure TVCLFileWatcher.SetFileMonitoring(const Value: string);
begin
...
procedure TVCLFileWatcher.SetMonitoring(const Value: TSWatchFilters);
begin
  FMonitoring := Value;
end;

procedure TVCLFileWatcher.SetOnException(const Value: TSFileExceptionEvent);
begin
  FOnException := Value;
end;

procedure TVCLFileWatcher.SetOnFileCriated(const Value: TSFileChangedEvent);
begin
  FOnFileCriated := Value;
end;

procedure TVCLFileWatcher.SetOnFileDeleted(const Value: TSFileChangedEvent);
begin
  FOnFileDeleted := Value;
end;


procedure TVCLFileWatcher.SetOnFileModified(const Value: TSFileChangedEvent);
begin
  FOnFileModified := Value;
end;

procedure TVCLFileWatcher.SetOnFileRemamed(const Value: TSFileRenamedEvent);
begin
  FOnFileRemamed := Value;
end;
procedure TVCLFileWatcher.SetSubTree(const Value: boolean);
begin
  FSubTree := Value;
end;

procedure TVCLFileWatcher.Start;
begin
...
procedure TVCLFileWatcher.Stop;
begin
  inherited;
  if Assigned(FWatchThread) then
    begin
...
procedure Register;
begin
  RegisterComponents('VCL Utils',[TVCLFileWatcher]);
end;
end.
